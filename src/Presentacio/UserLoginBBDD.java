...
public class UserLoginBBDD {
	...
	private JPanel getPanel() {
		...
		sl_panel.putConstraint(SpringLayout.NORTH, result_label, 10,
				SpringLayout.SOUTH, password);
		sl_panel.putConstraint(SpringLayout.SOUTH, login, -5,
				SpringLayout.NORTH, password);
		sl_panel.putConstraint(SpringLayout.EAST, password, 0,
				SpringLayout.EAST, login);
		sl_panel.putConstraint(SpringLayout.SOUTH, label_login, -11,
				SpringLayout.NORTH, label_password);
		sl_panel.putConstraint(SpringLayout.EAST, label_login, -6,
				SpringLayout.WEST, login);
		sl_panel.putConstraint(SpringLayout.NORTH, password, -3,
				SpringLayout.NORTH, label_password);
		JPanel panel = new JPanel(sl_panel);
		sl_panel.putConstraint(SpringLayout.WEST, result_label, 52,
				SpringLayout.WEST, panel);
		sl_panel.putConstraint(SpringLayout.SOUTH, result_label, -49,
				SpringLayout.SOUTH, panel);
		sl_panel.putConstraint(SpringLayout.EAST, result_label, -43,
				SpringLayout.EAST, panel);
		sl_panel.putConstraint(SpringLayout.WEST, label_password, 10,
				SpringLayout.WEST, panel);
		sl_panel.putConstraint(SpringLayout.WEST, label_login, 22,
				SpringLayout.WEST, panel);
		sl_panel.putConstraint(SpringLayout.EAST, login, -10,
				SpringLayout.EAST, panel);
		sl_panel.putConstraint(SpringLayout.WEST, password, 90,
				SpringLayout.WEST, panel);
		sl_panel.putConstraint(SpringLayout.WEST, login, 92, SpringLayout.WEST,
				panel);
		sl_panel.putConstraint(SpringLayout.NORTH, label_password, 74,
	...
	private boolean DemanarCredencials() {
		...
		while (!logat) {
			...
			Object[] options = { "Login In", "Jugar sense persistencia",
					"Cancelar" };

			int value = JOptionPane.showOptionDialog(null, getPanel(),
					"Connexio a la base de dades", JOptionPane.DEFAULT_OPTION,
